...
counts_file <- "data/nAcc_female-scz-counts.csv"
metadata_file <- "data/nAcc_female-scz-metadata.csv"

counts <- read_csv(counts_file) %>%
  column_to_rownames("symbol")
metadata <- read_csv(metadata_file) %>%
  select(`Sample-ID`, clinical_diagnosis) %>%
  mutate(diag = if_else(clinical_diagnosis == "Control", "CTL", "SCZ"))
...
out_filtered$complete_table %>%
  ...
  write_csv("results/female_scz_nacc_dge.csv")

save(out_filtered, file = "results/female_scz_nacc_dge.RData")
