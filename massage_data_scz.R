...
pheno <- read_csv(metadata) %>%
  filter(clinical_diagnosis %in% c("Schizophrenia", "Control"))
...
grouped_sections %>%
  map2(group_names, ~ write_csv(.x, str_glue("data/{.y}-scz-metadata.csv"))) %>%
  ...
  map2(group_names, ~ write_csv(.x, str_glue("data/{.y}-scz-counts.csv")))
