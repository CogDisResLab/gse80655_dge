...
pheno <- read_csv(metadata) %>%
  filter(clinical_diagnosis %in% c("Major Depression", "Control"))
...
grouped_sections %>%
  map2(group_names, ~ write_csv(.x, str_glue("data/{.y}-mdd-metadata.csv"))) %>%
  ...
  map2(group_names, ~ write_csv(.x, str_glue("data/{.y}-mdd-counts.csv")))
