# Female DLPFC DGE -- Bipolar

library(tidyverse)
library(edgeR)
source("process_dge_fun.R")

counts_file <- "data/DLPFC_female-bpd-counts.csv"
metadata_file <- "data/DLPFC_female-bpd-metadata.csv"
...
out_filtered$complete_table %>%
  ...
  write_csv("results/female_bpd_dlpfc_dge.csv")

save(out_filtered, file = "results/female_bpd_dlpfc_dge.RData")
