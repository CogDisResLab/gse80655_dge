# Female AnCg DGE - Bipolar

library(tidyverse)
library(edgeR)
source("process_dge_fun.R")

counts_file <- "data/AnCg_female-bpd-counts.csv"
metadata_file <- "data/AnCg_female-bpd-metadata.csv"
...
out_filtered$complete_table %>%
  ...
  write_csv("results/female_bpd_ancg_dge.csv")

save(out_filtered, file = "results/female_bpd_ancg_dge.RData")
