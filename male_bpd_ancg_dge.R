# Male AnCg DGE - Bipolar

library(tidyverse)
library(edgeR)
source("process_dge_fun.R")

counts_file <- "data/AnCg_male-bpd-counts.csv"
metadata_file <- "data/AnCg_male-bpd-metadata.csv"
...
out_filtered$complete_table %>%
  ...
  write_csv("results/male_bpd_ancg_dge.csv")

save(out_filtered, file = "results/male_bpd_ancg_dge.RData")
